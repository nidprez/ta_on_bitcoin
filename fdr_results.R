...
sig_rules2 <- length(sig_rules)
nonzero <- clusterres$nonzero

bootres <- as.matrix(bootres)[,nonzero]

f_bar <- clusterres$f_bar[nonzero]
omega <- clusterres$omega[nonzero]
...
### FDR+ #####
# Here we apply the FDR+ methodology from Barras, scaillet and Werner (2010) and Bajgrowicz, Scaillet (2012)

#get 2 sided pvalues for every rule
# bootres <- as.matrix(bootres)
pvals <- apply(as.matrix(1:s), 1, 
               function(x) mean(abs(f_bar[x]) < abs(bootres[, x]-f_bar[x])))

#Estimate the appropriate lambda (pvalue for which rules higher are 100% zero-alpha rules)
lambda <- seq(0.3,0.7,0.05)

#function to estimate pi0, the proportion of zero-alpha funds
estimate_pi0_hat <- function(pvals, lambda){
  (sum(pvals > lambda) / length(pvals)) * (1 / (1 - lambda))
}

#version for the bootstrap
boot_pi0 <- function(pvals, i, lambda){
  sapply(lambda, function(x) estimate_pi0_hat(pvals[i], x))
}

#bootstrap the different lambdas
boot_lambda <- boot(pvals, boot_pi0, 1000, lambda = lambda)

# for(i in seq_along(lambda)){
#   print(mean((boot$t[,i] - min(boot$t0))^2))
# }

#estimate the MSE for each lambda and choose the smallest one
MSE_lambda <- apply(boot_lambda$t, 2, function(x) mean((x - min(boot_lambda$t0))^2))
lambda <- lambda[order(MSE_lambda)[1]]
pi_0 <- boot_lambda$t0[order(MSE_lambda)[1]]

#estimate the appropriate gamma, cutoff value for pvalues
gamma <- seq(0.05,0.4,0.05)

#function for appropriate T(proportion of significant strategies)
...
#choose gamma with smallest MSE
...
calculate_FDR <- function(pvals, f_bar, pos = T, pi_0, gamma){
  ((1/2) * pi_0 * length(pvals) * gamma) / max(sum(pvals <= gamma & f_bar > 0), 1)
}
...
plot(f_bar)
points(ordered_p[1:index2],f_bar[ordered_p[1:index2]], col = "red")
